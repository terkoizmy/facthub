          ...
          <CardContent className="px-2 py-0 pb-2 h-1/4 flex flex-col ">
            ...
            <div className="flex flex-wrap gap-x-2 gap-y-[0.3rem] mt-2 text-xs ">
              {article.tags.map((_, index) =>
                index < 2 ? (
                  <Badge className="rounded-lg  dark:border-slate-700  dark:text-slate-600 dark:bg-slate-950">
                    {`#${_.toLocaleLowerCase()}`}
                  </Badge>
                ) : (
                  index === article.tags.length - 1 && (
                    <Badge className="rounded-lg dark:border-slate-700 dark:text-slate-600 dark:bg-slate-950">{`+${index - 1}`}</Badge>
                  )
                )
              )}
            </div>
